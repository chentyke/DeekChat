...
class ChatViewModel: ObservableObject {
    ...
    func sendMessage(_ content: String) async throws {
        ...
        do {
            ...
            try await ChatService.shared.sendMessage(currentChat?.messages ?? [],
                ...
                onReasoningUpdate: { [weak self] partialReasoning in
                    guard let self = self,
                          self.activeChat == chat.id
                    else { return }
                    ...
                    print("收到推理内容[\(partialReasoning.count)字符]: '\(partialReasoning)'")
                    reasoningContent += partialReasoning

                    // 更新推理内容
                    if var messages = self.currentChat?.messages {
                        messages[messages.count - 1].reasoningContent = reasoningContent
                        
                        // 明确设置消息状态为未完成，确保UI正确显示流式内容
                        messages[messages.count - 1].isComplete = false
